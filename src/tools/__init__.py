...
def search_runbooks(symptom: str) -> str:
    """
    Search runbooks for symptoms/patterns using keyword matching.
    
    Searches a comprehensive runbook database using fuzzy keyword matching.
    In production, this would integrate with a vector DB (Pinecone/Chroma)
    for semantic search over Confluence pages.
    
    Args:
        symptom: Symptom or error pattern to search for
        
    Returns:
        JSON string of matching runbooks with remediation steps
    """
    runbooks = {
        "high_memory_usage": {
    ...
    # Fuzzy keyword matching: score each runbook
    ...
    for score, key, runbook in scored_results[:3]:
        # Remove keywords from output (internal use only)
